...
<style>
  /* ✅ FULL WIDTH FORZADO (aunque el layout tenga max-w-*) */
  .chebs-full{
    width: 100% !important;
...
<!-- ✅ WRAPPER FULL ANCHO -->
<div class="chebs-full px-6 py-6">
...
function renderAuto(){
  ...
  const safe = q.length ? q.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') : '';
...
function ajustarAltoAuto(){
  if(!autoBox || autoBox.classList.contains('hidden')) return;

  const rect = inputProducto.getBoundingClientRect();
  const top = rect.bottom + 8;
  const espacioAbajo = window.innerHeight - top - 12;
...
function ajustarAltoAutoDebounced(){
  clearTimeout(window.__chebsAutoT);
  window.__chebsAutoT = setTimeout(ajustarAltoAuto, 30);
}
